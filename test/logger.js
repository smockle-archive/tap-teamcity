...
test('Logger.prototype.startTest', (t) => {
  ...
  // 5
  t.ok(pastTime <= logger.testStartTime && logger.testStartTime <= new Date().getTime(), 'stores valid time')
  // 6
  t.equal(result, `\n##teamcity[testSuiteStarted name='${test.name}']`, 'pushes test start string to output stream')
})
...
test('Logger.prototype.finishTest', (t) => {
  ...
  t.ok(regexpFront.test(result) && regexpBack.test(result), 'pushes test end string to output stream')
})
...
test('Logger.prototype.startAssertion', (t) => {
  ...
  // 11
  t.ok(pastTime <= logger.assertionStartTime && logger.assertionStartTime <= new Date().getTime(), 'stores valid time')
  // 12
  t.equal(result, `\n##teamcity[testStarted name='${assertion.name}' captureStandardOutput='true']`, 'pushes assertion start string to output stream')
})
...
test('Logger.prototype.finishAssertion', (t) => {
  ...
  const regexpBack = / duration='[0-9]*']$/
  t.ok(regexpFront.test(result) && regexpBack.test(result), 'pushes assertion end string to output stream')
})
