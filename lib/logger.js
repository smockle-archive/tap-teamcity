...
Logger.prototype.finishTest = function () {
  if (!this.test) return
  this.output.push(
    `\n##teamcity[testSuiteFinished name='${this.test
      .name}' duration='${timeSince(this.testStartTime)}']`
  )
}
...
Logger.prototype.startAssertion = function (assertion) {
  ...
  this.output.push(
    `\n##teamcity[testStarted name='${this.assertion
      .name}' captureStandardOutput='true']`
  )
}
...
Logger.prototype.finishAssertion = function () {
  this.output.push(
    `\n##teamcity[testFinished name='${this.assertion
      .name}' duration='${timeSince(this.assertionStartTime)}']`
  )
}
